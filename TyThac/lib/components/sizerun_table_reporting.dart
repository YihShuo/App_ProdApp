...
class SizeRunTableReporting extends StatefulWidget {
  final String apiAddress;
  final String order;
  final String partID;
  const SizeRunTableReporting({
    super.key,
  ...
  @override
  State<StatefulWidget> createState() => SizeRunTableReportingState();
}

class SizeRunTableReportingState extends State<SizeRunTableReporting> {
  ...
  Future<bool> loadTableData() async {
    ...
    tableColumnTitles.add(Container(
      ...
      child: Center(child: Text(AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableCycle, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold))),
    ));
    ...
    final sizeRunBody = await RemoteService().getOrderDispatchedSizeRun(
      widget.apiAddress,
      widget.order,
      widget.partID
    );
            ...
            children: [
              Padding(
                ...
                child: OutlinedButton(
                  ...
                  child: Center(
                    child: Text(AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableSelectAll, style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 20)),
                  )
                )
              ),
              ...
              Padding(
                ...
                child: OutlinedButton(
                      ...
                      builder: (BuildContext context) {
                        ...
                        if (isEmpty) {
                          return MessageDialog(
                            key: globalKey,
                            titleText: AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableDialogNotSelectTitle,
                            contentText: AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableDialogNotSelectContent,
                            showOKButton: true,
                            showCancelButton: false,
                            onPressed: () {
                              Navigator.of(context).popUntil((route) => route.settings.name == '/cutting_progress/reporting');
                            },
                          );
                        }
                        else {
                          return MessageDialog(
                            key: globalKey,
                            titleText: AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableDialogConfirmTitle,
                            contentText: AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableDialogConfirmContent,
                            showOKButton: true,
                            showCancelButton: true,
                            onPressed: () async {
                              globalKey.currentState?.changeContent(AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableDialogGeneratingTitle, const SizedBox(height: 72, child: Column(children: [CircularProgressIndicator(color: Colors.blue,),],),), false, false, () => {Navigator.of(context).popUntil((route) => route.settings.name == '/cutting_progress/reporting')}, true);
                              final prefs = await SharedPreferences.getInstance();
                              String userID = prefs.getString('userID') ?? '';
                              String department = prefs.getString('department') ?? '';
                              String factory = prefs.getString('factory') ?? '';

                              final body = await RemoteService().submitCuttingProgress(
                                widget.apiAddress,
                                widget.order,
                                userID,
                                department,
                              ...
                              if (jsonData['statusCode'] == 200) {
                                reloadTable();
                                globalKey.currentState?.changeContent(AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableDialogSuccessTitle, Text(AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableDialogSuccessContent), true, false, () => {Navigator.of(context).popUntil((route) => route.settings.name == '/cutting_progress/reporting')}, true);
                              }
                              else {
                                globalKey.currentState?.changeContent(AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableDialogFailedTitle, Text(AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableDialogFailedContent), true, false, () => {Navigator.of(context).popUntil((route) => route.settings.name == '/cutting_progress/reporting')}, true);
                              }
                            },
                          );
                        }
                  ...
                  child: Center(
                    child: Text(AppLocalizations.of(context)!.cuttingProgressReportingSizeRunTableDispatch, style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 20)),
                  )
                )
              )
            ],
