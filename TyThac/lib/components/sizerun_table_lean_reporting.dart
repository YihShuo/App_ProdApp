...
import 'package:production/components/selectable_lean_item.dart';
...
class SizeRunTableLeanReporting extends StatefulWidget {
  const SizeRunTableLeanReporting({
    super.key,
  ...
  @override
  State<StatefulWidget> createState() => SizeRunTableLeanReportingState();
}

class SizeRunTableLeanReportingState extends State<SizeRunTableLeanReporting> {
  List<String> sizeList = [], cycleList = [];
  List<Widget> tableColumnTitles = [];
  List<bool> sizeSelectAll = [], cycleSelectAll = [];
  List<List<GlobalKey<SelectableLeanItemState>>> itemKeys = [];
  ...
  Future<bool> loadTableData() async {
    ...
    for (int i = 0; i < tableFirstRow.length; i++) {
      cycleList.add(tableFirstRow[i]['Cycle']);
      cycleSelectAll.add(tableFirstRow[i]['AllDispatched']);
      GlobalKey<TableTitleState> titleKey = GlobalKey();
      rowTitleKeys.add(titleKey);
      List<GlobalKey<SelectableLeanItemState>> keyList = [];
      for (int j = 0; j < sizeList.length; j++) {
        GlobalKey<SelectableLeanItemState> key = GlobalKey();
        keyList.add(key);
      }
        ...
        child: HorizontalDataTable(
          ...
          leftSideItemBuilder: (BuildContext context, int index) {
            if (index < tableFirstRow.length) {
              ...
              int cycleInt = int.parse(cycle.substring(cycle.length - 3, cycle.length));
              GlobalKey<TableTitleState> titleKey = GlobalKey();
              rowTitleKeys[index] = titleKey;
              return GestureDetector(
                onTap: widget.readOnly == false ? () async {
                  GlobalKey<MessageDialogState> globalKey = GlobalKey();
                  if (cycleSelectAll[index] == false) {
                    showDialog(
                      barrierDismissible: false,
                      context: context,
                      builder: (BuildContext context) {
                    ...
                    final body = await RemoteService().submitLeanSectionProgress(
                      ...
                      widget.section,
                      widget.type,
                      cycleList.length > 1 ? cycle : cycle.replaceAll('-001', ''),
                      '',
                      0,
                      '${widget.building}_${widget.lean}',
                      'Completed'
                ...
                for (int j = 0; j < tableContentRows[index]['Parts'][0]['SizeQty'].length; j++) {
                  if (tableContentRows[index]['Parts'][0]['SizeQty'][j]['Size'].toString() == sizeList[i]) {
                    if (int.parse(tableContentRows[index]['Parts'][0]['SizeQty'][j]['Qty'].toString()) > 0) {
                      GlobalKey<SelectableLeanItemState> globalKey = GlobalKey();
                      ...
                      cells.add(
                        Container(
                          ...
                          child: SelectableLeanItem(
                            ...
                            size: sizeList[i],
                            pairs: tableContentRows[index]['Parts'][0]['SizeQty'][j]['Qty'],
                            shortage: tableContentRows[index]['Parts'][0]['SizeQty'][j]['Shortage'],
                            text: tableContentRows[index]['Parts'][0]['SizeQty'][j]['Qty'].toString(),
                            selectable: !widget.readOnly,
                            selected: tableContentRows[index]['Parts'][0]['SizeQty'][j]['Dispatched'],
                            checkSizeRunStatus: checkSizeRunStatus,
                            userID: '${widget.building}_${widget.lean}',
                          ),
                        )
                      );
                    }
  ...
  void checkSizeRunStatus(String size, String cycle) {
    ...
    allCompleted = true;
    index = cycleList.indexOf(cycle == widget.order ? '$cycle-001' : cycle);
    for (int i = 0; i < tableColumnTitles.length - 1; i++) {
      if (itemKeys[index][i].currentContext != null && itemKeys[index][i].currentState!.state == false) {
        allCompleted = false;
        break;
      }
    }
    cycleSelectAll[index] = allCompleted;
    rowTitleKeys[index].currentState?.changeStatus(cycleSelectAll[index]);
  }
