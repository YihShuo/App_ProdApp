...
import 'package:production/components/sizerun_table.dart';

String apiAddress = '', locale = 'zh';

class CuttingWorkOrderDispatch extends StatefulWidget {
  const CuttingWorkOrderDispatch({super.key});

  @override
  State<StatefulWidget> createState() => CuttingWorkOrderDispatchState();
}

class CuttingWorkOrderDispatchState extends State<CuttingWorkOrderDispatch> with SingleTickerProviderStateMixin {
  String order = '';
  String lean = '';
  ...
  Future<List<List<String>>> fetchTabs() async {
    ...
    final body = await RemoteService().getOrderPart(
      apiAddress,
      order
    );
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Scaffold(
            appBar: AppBar(
              leading: IconButton(
                ...
                onPressed: () {
                  Navigator.of(context).popUntil((route) => route.settings.name == '/cutting');
                },
              ),
          ...
          if (tabs.isNotEmpty) {
                      ...
                      onPressed: () {
                        showDialog(
                          ...
                          builder: (BuildContext context) {
                            return CuttingPartSettingDialog(
                              ry: order,
                              reloadAutoCuttingPart: refreshPage
                            );
                          },
                        );
                      },
            ...
            return Scaffold(
              ...
              body: Center(
                child: Text(AppLocalizations.of(context)!.cuttingWorkOrderDispatchNoData)
              )
            );
          }
        } else {
                    ...
                    Row(
                      children: [
                        ...
                        OutlinedButton(
                                ...
                                if (part != '' && cycle != '') {
                                  return MessageDialog(
                                    ...
                                    onPressed: () async {
                                      ...
                                      final body = await RemoteService().generateCuttingGroupWorkOrder(
                                        apiAddress,
                                        widget.order,
                                        part,
                                        cycle,
                                      ...
                                      if (jsonData['statusCode'] == 200) {
                                        widget.refreshPage();
                                        globalKey.currentState?.changeContent(AppLocalizations.of(context)!.successTitle, Text(AppLocalizations.of(context)!.successContent), true, false, () => {Navigator.of(context).popUntil((route) => route.settings.name == '/cutting/dispatch')}, true);
                                      }
                                  ...
                                  return MessageDialog(
                                    ...
                                    contentText: AppLocalizations.of(context)!.dispatchNoSelection,
                                    showOKButton: true,
                          ...
                          child: Center(
                            child: Text(AppLocalizations.of(context)!.dispatch, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
                          )
                        )
                      ],
                    )
...
class CuttingPartSettingDialog extends StatefulWidget {
  const CuttingPartSettingDialog({
    super.key,
    required this.ry,
  ...
  @override
  State<StatefulWidget> createState() => CuttingPartSettingDialogState();
}

class CuttingPartSettingDialogState extends State<CuttingPartSettingDialog> {
  ...
  Future<void> loadPart() async {
    ...
    final jsonPart = json.decode(partBody);
    for (int i = 0; i < jsonPart.length; i++) {
      if (jsonPart[i]['PartName'][0]['Type'] != 'Cutting') {
        selectedPart.add(jsonPart[i]["PartID"]);
      }
      partItems.add(
        CheckboxListItem(
          index: i,
          selected: jsonPart[i]['PartName'][0]['Type'] != 'Cutting',
    ...
    return AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            onPressed: () async {
              final body = await RemoteService().setCuttingPart(
                apiAddress,
                widget.ry,
                "'${selectedPart.join("','")}'",
              );
