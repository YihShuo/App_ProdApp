...
class ProcessWorkOrderReporting extends StatefulWidget {
  const ProcessWorkOrderReporting({super.key});

  @override
  State<StatefulWidget> createState() => ProcessWorkOrderReportingState();
}

class ProcessWorkOrderReportingState extends State<ProcessWorkOrderReporting> {
  ...
  Future<bool> loadTableData() async {
    ...
    try {
      ...
      tableColumnTitles.add(Container(
        ...
        child: Center(child: Text(AppLocalizations.of(context)!.processProgressReportingCycle, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold))),
      ));
      ...
      final sizeRunBody = await RemoteService().getOrderProcessingDispatchedSizeRun(
        apiAddress,
        order,
        section
      );
              ...
              children: [
                Padding(
                  ...
                  child: OutlinedButton(
                    ...
                    child: Center(
                      child: Text(AppLocalizations.of(context)!.processProgressReportingSelectAll, style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 20)),
                    )
                  )
                ),
                ...
                Padding(
                  ...
                  child: OutlinedButton(
                        ...
                        builder: (BuildContext context) {
                          ...
                          if (isEmpty) {
                            return MessageDialog(
                              key: globalKey,
                              titleText: AppLocalizations.of(context)!.processProgressReportingDialogNotSelectTitle,
                              contentText: AppLocalizations.of(context)!.processProgressReportingDialogNotSelectContent,
                              showOKButton: true,
                              showCancelButton: false,
                              onPressed: () {
                                Navigator.of(context).popUntil((route) => route.settings.name == '/process_progress/chart/reporting');
                              },
                            );
                          }
                          else {
                            return MessageDialog(
                              ...
                              onPressed: () async {
                                globalKey.currentState?.changeContent(AppLocalizations.of(context)!.processProgressReportingDialogGeneratingTitle, const SizedBox(height: 72, child: Column(children: [CircularProgressIndicator(color: Colors.blue),],),), false, false, () => {Navigator.of(context).popUntil((route) => route.settings.name == '/process_progress/chart/reporting')}, true);
                                final prefs = await SharedPreferences.getInstance();
                                String userID = prefs.getString('userID') ?? '';
                                String department = prefs.getString('department') ?? '';
                                String factory = prefs.getString('factory') ?? '';

                                final body = await RemoteService().submitProcessingProgress(
                                  apiAddress,
                                  order,
                                  userID,
                                  department,
                                ...
                                if (jsonData['statusCode'] == 200) {
                                  myFuture = loadTableData();
                                  globalKey.currentState?.changeContent(AppLocalizations.of(context)!.processProgressReportingDialogSuccessTitle, Text(AppLocalizations.of(context)!.processProgressReportingDialogSuccessContent), true, false, () => {Navigator.of(context).popUntil((route) => route.settings.name == '/process_progress/chart/reporting')}, true);
                                }
                                else {
                                  globalKey.currentState?.changeContent(AppLocalizations.of(context)!.processProgressReportingDialogFailedTitle, Text(AppLocalizations.of(context)!.processProgressReportingDialogFailedContent), true, false, () => {Navigator.of(context).popUntil((route) => route.settings.name == '/process_progress/chart/reporting')}, true);
                                }
                              },
                            );
                          }
                    ...
                    child: Center(
                      child: Text(AppLocalizations.of(context)!.processProgressReportingDispatch, style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 20)),
                    )
                  )
                )
              ],
          ...
          return Scaffold(
            appBar: AppBar(
              leading: IconButton(
                ...
                onPressed: () {
                  Navigator.of(context).popUntil((route) => route.settings.name == '/process_progress/chart');
                },
              ),
