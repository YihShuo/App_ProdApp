...
class ProcessProgressChart extends StatefulWidget {
  const ProcessProgressChart({super.key});

  @override
  State<StatefulWidget> createState() => ProcessProgressChartState();
}

class ProcessProgressChartState extends State<ProcessProgressChart> {
  String order = '';
  String userName = '';
  String group = '';
  ...
  Future<bool> loadProcessPart() async {
    ...
    try {
      ...
      final body = await RemoteService().getProcessingReportingFlow(
        apiAddress,
        order,
      );
      ...
      while (sectionList.isNotEmpty) {
        for (int i = 0; i < sectionList.length; i++) {
          if (dispatchedList.contains(sectionList[i][2])) {
            graph.addEdge(Node.Id(sectionList[i][2]), Node.Id(sectionList[i][0]), paint: Paint()..strokeWidth = (1.5)..color = const Color.fromRGBO(120, 120, 120, 1));
            dispatchedLevel.add(dispatchedLevel[dispatchedList.indexOf(sectionList[i][2])] + 1);
            dispatchedList.add(sectionList[i][0]);
            dispatchedName.add(sectionList[i][1]);
          ...
          return Scaffold(
            appBar: AppBar(
              leading: IconButton(
                ...
                onPressed: () {
                  Navigator.of(context).popUntil((route) => route.settings.name == '/process_progress');
                },
              ),
                        ...
                        return Visibility(
                          ...
                          child: Card(
                            ...
                            child: InkWell(
                              ...
                              ? () {
                                  Navigator.pushNamed(context, '/process_progress/chart/reporting', arguments: '$order;$section;$sectionName');
                                }
                              : null,
                              child: Column(
                                children: [
