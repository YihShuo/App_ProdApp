...
class CuttingWorkOrder extends StatefulWidget {
  const CuttingWorkOrder({super.key});

  @override
  CuttingWorkOrderState createState() => CuttingWorkOrderState();
}

class CuttingWorkOrderState extends State<CuttingWorkOrder> {
  ...
  void getOrders() async {
    orderList = [];
    try {
      final body = await RemoteService().getMonthOrder(
        apiAddress,
        '${sFactory}_$sLean',
        selectedMonth,
        sType,
    ...
    if (jsonData.length > 0) {
      for (int i = 0; i < jsonData.length; i++) {
        orders.add(Padding(
          ...
          child: InkWell(
            onTap: () {
              Navigator.pushNamed(context, '/cutting/dispatch', arguments: jsonData[i]["Order"] + ';${sFactory}_$sLean');
            },
            child: Ink(
              width: screenWidth,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(AppLocalizations.of(context)!.cuttingWorkOrderTitle),
    ...
    return AlertDialog(
      ...
      content: Column(
        children: [
          ...
          DropdownButton<String>(
            ...
            items: [
              DropdownMenuItem(
                value: 'Incomplete',
                child: Center(
                  child: Text(AppLocalizations.of(context)!.cuttingWorkOrderFilterIncomplete),
                )
              ),
              DropdownMenuItem(
                value: 'All',
                child: Center(
                  child: Text(AppLocalizations.of(context)!.cuttingWorkOrderFilterAll),
                )
              )
            ],
            onChanged: (value) {
          ...
          Align(
            alignment: Alignment.centerLeft,
            child: Text(AppLocalizations.of(context)!.cuttingWorkOrderFilterDate)
          ),
          SizedBox(
            height: 40,
            child: TextField(
          ...
          Align(
            alignment: Alignment.centerLeft,
            child: Text(AppLocalizations.of(context)!.cuttingWorkOrderFilterFactory)
          ),
          DropdownButton<String>(
            isExpanded: true,
            underline: Container(
